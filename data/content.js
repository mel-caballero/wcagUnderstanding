const wcagObj = {
  ...
  { // 1.1.1.
    ...
    "goal" : 
    {
      "cat" : "Fer que la informació transmesa mitjançant contingut no textual sigui accessible mitjançant alternatives textuals.", 
      "es" : "Hacer que la información transmitida mediante contenido no textual sea accesible a través de alternativas textuales.", 
      "en" : "Make the information transmitted through non-textual content accessible through alternative textual.",
    },
    "intent" : 
    {
    ...
    "benefits" :
    {
      "cat" : "Ajuda les persones que tenen dificultats per percebre el contingut visual.# Les alternatives de text poden ajudar algunes persones que tenen dificultats per comprendre el significat de fotografies, dibuixos i altres imatges (p. ex., dibuixos lineals, dissenys gràfics, pintures , tres (representacions dimensionals), gràfics, taules, animacions, etc.# Les persones sordcegues poden llegir el text en braille.", 
      
      "es" : "Ayuda a las personas que tienen dificultades para percibir el contenido visual.# Las alternativas de texto pueden ayudar a algunas personas que tienen dificultades para comprender el significado de fotografías, dibujos y otras imágenes (p. Ej., Dibujos lineales, diseños gráficos, pinturas, tres -representaciones dimensionales), gráficos, tablas, animaciones, etc.# Las personas sordociegas pueden leer el texto en braille.", 
      
      "en" : "Helps people who have difficulty perceiving visual content # Text alternatives can help some people who have difficulty understanding the meaning of photographs, drawings, and other images (e.g., line drawings, graphic designs, paintings , three-dimensional representations), graphs, tables, animations, etc. # Deafblind people can read braille text.",
    },
    "verification" :
    {
      "cat" : "A. Tot contingut no textual ha de tenir una alternativa textual breu que proporcioni la mateixa informació o compleixi el mateix propòsit que el contingut no textual.#B. Les alternatives textuals s'han d'actualitzar quan canvieu el contingut no textual.#C. Les imatges incloses des de CSS han de ser purament decoratives i no han d'aportar informació tret que hi hagi alternativa textual a la pàgina.#D. En els grups d'imatges que transmeten una única informació de forma conjunta es proporciona en una sola de les imatges descrivint tot el grup, deixant la resta com a decoratives.#E. L'art ASCII, les emoticones i el leetspeak han de disposar d'alternatives textuals immediatament abans o després que transmetin la mateixa informació. Si amb una alternativa textual curta no és suficient per transmetre la mateixa informació o complir el mateix propòsit que el contingut no textual, aleshores a l'alternativa textual curta s'ha de proporcionar una breu descripció del contingut no textual ia més es proporciona una alternativa textual llarga que sí que transmeti la mateixa informació o compleixi el mateix propòsit.#G. Les imatges que tenen alguna funcionalitat com els enllaços, àrees actives de mapes d'imatge o botons de tipus imatge als formularis, tenen un text alternatiu que n'identifica el propòsit.#H. Els enllaços acompanyats d'imatges que actuen com a enllaç al mateix recurs han d'estar combinats en un únic enllaç. El contingut d'àudio, vídeo o multimèdia (incloent contingut en directe), els tests o exercicis que podrien ser invàlids si es mostren com a text, així com el contingut la funció principal del qual és produir una experiència sensorial específica (música, quadres, etc.) han de tenir una alternativa textual breu com a etiqueta descriptiva que almenys identifiqui quin és el contingut no textual.#J. Els CAPTCHA han de disposar d'una alternativa textual que descriu el propòsit del CAPTCHA ia més ha d'existir un CAPTCHA alternatiu que compleix el mateix propòsit però en una modalitat diferent. #K. Les imatges decoratives han de tenir un text alternatiu buit i no tenir un títol, o bé s'hi inclouen des dels fulls d'estil, de manera que puguin ser ignorats pels productes de suport.", 
      
      "es" : "A. Todo contenido no textual debe disponer de una alternativa textual breve que proporcione la misma información o cumpla el mismo propósito que el contenido no textual.#B. Las alternativas textuales se deben actualizar cuando cambie el contenido no textual.#C. Las imágenes incluidas desde CSS deben ser puramente decorativas y no deben aportar información a no ser que exista alternativa textual en la página.#D. En los grupos de imágenes que transmiten una única información de forma conjunta se proporciona en una sola de las imágenes describiendo todo el grupo, dejando el resto como decorativas.#E. El arte ASCII, los emoticonos y el leetspeak deben disponer de alternativas textuales inmediatamente antes o después que transmitan la misma información.#F. Si con una alternativa textual corta no es suficiente para transmitir la misma información o cumplir el mismo propósito que el contenido no textual, entonces en la alternativa textual corta se debe proporcionar una breve descripción del contenido no textual y además se proporciona una alternativa textual larga que sí transmita la misma información o cumpla el mismo propósito.#G. Las imágenes que tienen alguna funcionalidad como los enlaces, áreas activas de mapas de imagen o botones de tipo imagen en los formularios, tienen un texto alternativo que identifica su propósito.#H. Los enlaces acompañados de imágenes que actúan como enlace al mismo recurso deben estar combinados en un único enlace.#I. El contenido de audio, vídeo o multimedia (incluyendo contenido en directo), los test o ejercicios que podrían ser inválidos si se muestran como texto, así como el contenido cuya función principal es producir una experiencia sensorial específica (música, cuadros, etc.) deben tener una alternativa textual breve a modo de etiqueta descriptiva que al menos identifique cuál es el contenido no textual.#J. Los CAPTCHAs deben disponer de una alternativa textual que describe el propósito del CAPTCHA y además debe existir un CAPTCHA alternativo que cumple el mismo propósito pero en una modalidad diferente.#K. Las imágenes decorativas deben tener un texto alternativo vacío y carecer de un título, o bien se incluyen desde las hojas de estilo, de forma que puedan ser ignoradas por los productos de apoyo.", 
      
      "en" : "A. All non-text content must have a short text alternative that provides the same information or serves the same purpose as non-text content.#B. Textual alternatives must be updated when non-textual content changes.#C. The images included from CSS must be purely decorative and must not provide information unless there is a textual alternative on the page.#D. In groups of images that convey a single information together, it is provided in a single image describing the entire group, leaving the rest as decorative.#E. ASCII art, emoticons, and leetspeak must have textual alternatives immediately before or after that convey the same information.#F. If a short text alternative is not sufficient to convey the same information or serve the same purpose as non-text content, then a short description of the non-text content must be provided in the short text alternative and also a long text alternative is provided that if conveys the same information or serves the same purpose.#G. Images that have some functionality such as links, image map hotspots or image type buttons on forms, have an alternative text that identifies their purpose.#H. The links accompanied by images that act as a link to the same resource must be combined in a single link.#I. Audio, video or multimedia content (including live content), tests or exercises that could be invalid if displayed as text, as well as content whose main function is to produce a specific sensory experience (music, paintings, etc.) they should have a short textual alternative as a descriptive tag that at least identifies what the non-textual content is.#J. CAPTCHAs must have a textual alternative that describes the purpose of the CAPTCHA, and there must also be an alternative CAPTCHA that fulfills the same purpose but in a different modality.#K. Decorative images must have empty alt text and no title or are included from style sheets so that they can be ignored by assistive products.",
    }, 
    "tools" :
    {
      "cat" : "A. Web Developer Toolbar.#1. Images - Outline images without alt attributes > Comprovar si hi ha alguna imatge ressaltada, és un error i se li ha de proporcionar un atribut “alt”.#2. Images - Display alt attributes > Valorar si el text d'aquest atribut descriu la funció de la imatge.#B. Web Developer Toolbar.#1. Images - Display alt attributes > Valoreu si el text d'aquest atribut s'adapta als canvis en el contingut no textual.#C.Web Developer Toolbar. Images - Outline all images > Verificar que les imatges introduïdes des de les fulles d'estil (les que no es mostren ressaltades) no transmeten informació important. Si alguna imatge transmet informació important, verificar que la informació està disponible per a les eines de suport.#C. Web Developer Toolbar. Images - Display alt attributes > Localizar las imágenes que actúan como un grupo y verificar que se proporciona un texto alternativo adecuado en una de las imágenes y que el resto tienen un texto alternativo vacío.#E. Web Developer Toolbar. Images - Display alt attributes > Localitzar els elements que actuen art ASCII, emoticones i leetspeak i verificar que es proporciona un text alternatiu adequat immediatament abans o després.", 

      "es" : "A. Web Developer Toolbar.#1. Images - Outline images without alt attributes > Si hay alguna imagen resaltada, es un error y se le debe proporcionar un atributo “alt”.#2. Images - Display alt attributes > Valorar si el texto describe la función de la imagen.#B. Web Developer Toolbar.#1. Images - Display alt attributes > Valorar si el texto se adapta a los cambios en el contenido no textual.#C.Web Developer Toolbar. Images - Outline all images > Verificar que las imágenes introducidas desde CSS (las que no se muestran resaltadas) no transmiten información importante. Si alguna imagen transmite información importante, verificar que la información está disponible para las herramientas de apoyo.#C. Web Developer Toolbar. Images - Display alt attributes > Localizar las imágenes que actúan como un grupo y verificar que se proporciona un texto alternativo adecuado en una de las imágenes y que el resto tienen un texto alternativo vacío.#E. Web Developer Toolbar. Images - Display alt attributes > Localizar los elementos que actúan arte ASCII, emoticonos y leetspeak y verificar que se proporciona un texto alternativo adecuado inmediatamente antes o después.#F. Web Developer Toolbar. Images - Display alt attributes > Localizar las imágenes con un contenido complejo (ej. infografías, esquemas) y verificar que tienen alternativa textual con breve descripción del contenido. Si la descripción es larga, verificar que dispone de atributo 'longdesc', que existe descripción en el contenido de la páginao que inmediatamente antes o después de la imagen se proporciona un enlace a otra página con la descripción textual.#G. Web Developer Toolbar. Images - Display alt attributes > Localizar imágenes que actúan como enlace. Si solamente es una imagen verificar que tiene texto alternativo que identifica función y destino. Si contiene texto, incluirlo.#H. Web Developer Toolbar. Images - Display alt attributes > Localizar ima´genes que actúan como enlaces y verificar que no existan enlaces consecutivos al mismo recurso.#I. Web Developer Toolbar. Images - Display alt attributes > Verificar que los contenidos de esta categoría disponen de una alternativa textual adecuada..#J. Web Developer Toolbar. Images - Display alt attributes > Verificar que los captchas tienen una alternativa textual adecuada y comprobar que existe alternativa en una modalidad diferente.#K. Web Developer Toolbar. Images - Display alt attributes > Localizar imagénes decorativas y verificar que el texto alternativo está vacío y no disponen de título.", 

      "en" : "A. Web Developer Toolbar.#1. Images - Outline images without alt attributes > Checking for any highlighted images is an error and should be given a “alt“ attribute.#2. Images - Display alt attributes > Check whether the text of this attribute describes the function of the image.#B. Web Developer Toolbar.#1. Images - Display alt attributes > CHeck whether the text of the attribute adapts to changes in the non-textual content.#C.Web Developer Toolbar. Images - Outline all images > Verify that the images entered from the style sheets (those that are not highlighted) do not convey important information. If any image conveys important information, verify that the information is available to support tools.#C. Web Developer Toolbar. Images - Display alt attributes > Locate the images that act as a group and verify that a suitable alt text is provided in one of the images and that the rest have empty alt text.#E. Web Developer Toolbar. Images - Display alt attributes > Locate the elements that act as ASCII art, emoticons, and leetspeak and verify that a suitable alternative text is provided immediately before or after.",
    },
    "category" : ["content"]
  },
